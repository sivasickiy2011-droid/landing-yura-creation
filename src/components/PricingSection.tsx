...
import MarketplaceCard from "@/components/pricing/MarketplaceCard";
...
const PricingSection = () => {
  ...
  const marketplacePriceMonth = prices 
    ? prices.marketplace.priceMonth.replace(/(\d)(?=(\d{3})+$)/g, '$1 ')
    : '2 990';
  
  const marketplacePriceYear = prices
    ...
    >
      <div className="max-w-[1460px] mx-auto">
        ...
        <MarketplaceCard
          billingPeriod={billingPeriod}
          loading={loading}
          priceMonth={marketplacePriceMonth}
          priceYear={marketplacePriceYear}
          isVisible={isVisible}
          onSelectMarketplace={() => handleSelectTariff("Маркетплейс")}
        />
      </div>
